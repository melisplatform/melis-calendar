<?php 
	return array(
	    'tr_melistoolcalendar_tool_calendar' => 'Calendar',
	    'tr_melistoolcalendar_tool_calendar_description' => 'This tool allows to place events in a calendar in order to display them on the dashboard.',
	    
	    // Dashborad plugin
	    'tr_PluginSection_meliscalendar' => 'Melis Calendar',
	    'tr_meliscalendar_dashboard' => 'Calendar',
	    'tr_meliscalendar_dashboard_description' => 'This tool allows to place events in a calendar in order to display them on the dashboard.',
	    
	    // Save Event Messages
	    'tr_melistoolcalendar_form_event_title' => 'Title',
	    'tr_melistoolcalendar_form_event_title tooltip' => 'Title of the event to place on the calendar',
	    'tr_melistoolcalendar_form_create' => 'Create new event',
	    'tr_melistoolcalendar_form_create_drag' => '*Drag and drop after you entered the event title.',
	    'tr_melistoolcalendar_form_error_msg' => 'Please enter event title before dragging to calendar',
	    'tr_melistoolcalendar_save_event_title' => 'Save calendar event',
	    'tr_melistoolcalendar_save_event_success' => 'Calendar event has been successfully saved',
	    
	    // Event Deletion Messages
	    'tr_melistoolcalendar_delete_event_title' => 'Delete event',
	    'tr_melistoolcalendar_delete_event_confirm' => 'Are you sure you want to delete this event?',
	    'tr_melistoolcalendar_delete_event_success' => 'Event has been successfully deleted',
	    'tr_melistoolcalendar_delete_event_btn_yes' => 'Yes',
	    'tr_melistoolcalendar_delete_event_btn_no' => 'No',
	    'tr_melistoolcalendar_delete_event_unable' => 'Unable to delete event',
	    
	    // Recent Event Added
	    'tr_melistoolcalendar_recent_added' => 'Recent events added',
	    
	    // Months and days
	    'tr_melistoolcalendar_fullcalendar_jan' => 'January',
	    'tr_melistoolcalendar_fullcalendar_feb' => 'February',
	    'tr_melistoolcalendar_fullcalendar_mar' => 'March',
	    'tr_melistoolcalendar_fullcalendar_apr' => 'April',
	    'tr_melistoolcalendar_fullcalendar_may' => 'May',
	    'tr_melistoolcalendar_fullcalendar_jun' => 'June',
	    'tr_melistoolcalendar_fullcalendar_jul' => 'July',
	    'tr_melistoolcalendar_fullcalendar_aug' => 'August',
	    'tr_melistoolcalendar_fullcalendar_sep' => 'September',
	    'tr_melistoolcalendar_fullcalendar_oct' => 'October',
	    'tr_melistoolcalendar_fullcalendar_nov' => 'November',
	    'tr_melistoolcalendar_fullcalendar_dec' => 'December',
	    'tr_melistoolcalendar_fullcalendar_sun' => 'Sun',
	    'tr_melistoolcalendar_fullcalendar_mon' => 'Mon',
	    'tr_melistoolcalendar_fullcalendar_tue' => 'Tue',
	    'tr_melistoolcalendar_fullcalendar_wed' => 'Wed',
	    'tr_melistoolcalendar_fullcalendar_thu' => 'Thu',
	    'tr_melistoolcalendar_fullcalendar_fri' => 'Fri',
	    'tr_melistoolcalendar_fullcalendar_sat' => 'Sat',
	    
	    'tr_melistoolcalendar_fullcalendar_today' => 'Today',
	);
